...
class AccidentTwoWays(Accident):
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True, timeout=180):
        ...
        # New parameters for the lurching car
        self._lurch_delay = get_value_parameter(config, 'lurch_delay', float, 1.0) # Delay after ego is in trigger distance
        self._lurch_distance = get_value_parameter(config, 'lurch_distance', float, 1.5) # Distance the car lurches
        self._lurch_speed = get_value_parameter(config, 'lurch_speed', float, 5.0) # Speed of the lurch
    ...
    def _create_behavior(self):
        ...
        if self.route_mode:
            ego_behavior.add_child(SwitchWrongDirectionTest(False))
            ego_behavior.add_child(ChangeOppositeBehavior(active=False))
        ego_behavior.add_child(OppositeActorFlow(reference_wp, self.ego_vehicles[0], self._opposite_interval))
        ego_behavior.add_child(WaitForever()) # Keep this branch active until the ego passes the end_wp

        end_condition.add_child(ego_behavior)

        # First actor lurch behavior: The vehicle that was seemingly static suddenly moves
        first_actor = self.other_actors[1] # police_car is 0, first_actor is 1, second_actor is 2
        lurch_wp_transform = self._move_waypoint_forward(self._first_vehicle_wp, self._lurch_distance).transform

        first_actor_lurch_behavior = py_trees.composites.Sequence(name="First Actor Lurch Behavior")
        # Trigger when ego is close to the first accident vehicle
        first_actor_lurch_behavior.add_child(InTriggerDistanceToLocation(
            self.ego_vehicles[0], self._first_vehicle_wp.transform.location, self._trigger_distance))
        first_actor_lurch_behavior.add_child(Idle(self._wait_duration + self._lurch_delay)) # Wait a bit after ego perceives it
        first_actor_lurch_behavior.add_child(HandBrakeVehicle(first_actor, False)) # Release handbrake
        
        # A small lurch forward
        first_actor_lurch_behavior.add_child(
            BasicAgentBehavior(first_actor, lurch_wp_transform.location, target_speed=self._lurch_speed, opt_dict={'ignore_traffic_lights': True, 'ignore_stop_signs': True}))
        first_actor_lurch_behavior.add_child(HandBrakeVehicle(first_actor, True)) # Re-apply handbrake
        first_actor_lurch_behavior.add_child(WaitForever()) # Keep this branch alive

        end_condition.add_child(first_actor_lurch_behavior)
...
class HazardAtSideLaneTwoWays(HazardAtSideLane):
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True, timeout=180):
        ...
        # New parameters for the bicycle stop
        self._bicycle_stop_distance_ratio = get_value_parameter(config, 'bicycle_stop_distance_ratio', float, 0.5) # Ratio of _bicycle_drive_distance to trigger stop
        self._bicycle_stop_duration = get_value_parameter(config, 'bicycle_stop_duration', float, 0.5) # Duration of the stop in seconds
    ...
    def _create_behavior(self):
        ...
        # Bicycle movement. Modify the first bicycle to stop abruptly.
        offset = self._offset * self._starting_wp.lane_width / 2
        opt_dict = {'offset': offset}

        # First bicycle (self.other_actors[0]) with sudden stop
        bicycle_0 = self.other_actors[0]
        target_loc_0 = self._target_locs[0]
        
        bicycle_0_behavior = py_trees.composites.Sequence(name="Bicycle 0 behavior (with stop)")
        # Initial movement
        bicycle_0_behavior.add_child(BasicAgentBehavior(bicycle_0, target_loc_0, target_speed=self._bicycle_speed, opt_dict=opt_dict))
        
        # Trigger the stop after driving a certain distance
        trigger_stop_distance = self._bicycle_drive_distance * self._bicycle_stop_distance_ratio
        bicycle_0_behavior.add_child(DriveDistance(bicycle_0, trigger_stop_distance)) # Atomic to wait until bicycle drives this distance
        
        bicycle_0_behavior.add_child(HandBrakeVehicle(bicycle_0, True)) # Apply handbrake
        bicycle_0_behavior.add_child(Idle(self._bicycle_stop_duration)) # Brief stop
        bicycle_0_behavior.add_child(HandBrakeVehicle(bicycle_0, False)) # Release handbrake
        
        # Continue to the original target location
        bicycle_0_behavior.add_child(BasicAgentBehavior(bicycle_0, target_loc_0, target_speed=self._bicycle_speed, opt_dict=opt_dict))
        
        bicycle_0_behavior.add_child(HandBrakeVehicle(bicycle_0, 1)) # Final handbrake to ensure it stays stopped
        bicycle_0_behavior.add_child(WaitForever()) # Keep this branch alive
        main_behavior.add_child(bicycle_0_behavior)

        # Other bicycles remain unchanged (bicycle_1, self.other_actors[1])
        if len(self.other_actors) > 1:
            bicycle_1 = self.other_actors[1]
            target_loc_1 = self._target_locs[1]
            bicycle_1_behavior = py_trees.composites.Sequence(name="Bicycle 1 behavior")
            bicycle_1_behavior.add_child(BasicAgentBehavior(bicycle_1, target_loc_1, target_speed=self._bicycle_speed, opt_dict=opt_dict))
            bicycle_1_behavior.add_child(HandBrakeVehicle(bicycle_1, 1))
            bicycle_1_behavior.add_child(WaitForever())
            main_behavior.add_child(bicycle_1_behavior)

        # Original behavior for ego and opposite traffic management
        behavior = py_trees.composites.Sequence(name="Side lane behavior")
        behavior.add_child(InTriggerDistanceToVehicle(
            self.ego_vehicles[0], self.other_actors[0], self._trigger_distance))
        behavior.add_child(Idle(self._wait_duration))
        ...
        root.add_child(main_behavior)
        if self.route_mode:
            root.add_child(SwitchWrongDirectionTest(True)) # Re-enable wrong direction test
