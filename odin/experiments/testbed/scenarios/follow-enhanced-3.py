...
class FollowLeadingVehicle(BasicScenario):
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True,
        ...
        # NEW: Parameters for additional actors to decrease driveability
        self._parked_car_distance = 20  # meters from ego start (trigger point)
        self._pedestrian_spawn_distance = 35 # meters from ego start
        self._pedestrian_cross_speed = 1.5 # m/s
        self._pedestrian_spawn_transform = None
    ...
    def _initialize_actors(self, config):
        ...
        self.other_actors.append(first_vehicle) # other_actors[0]

        # NEW: Parked vehicle (static obstacle)
        parked_waypoint, _ = get_waypoint_in_distance(self._reference_waypoint, self._parked_car_distance)
        # Offset to the side of the road (e.g., right side)
        parked_transform = parked_waypoint.transform
        right_vector = parked_transform.get_right_vector()
        # Place it slightly off the lane, adjust Z for ground level
        parked_transform.location += carla.Location(x=right_vector.x * 2.5, y=right_vector.y * 2.5, z=0.5)
        parked_car = CarlaDataProvider.request_new_actor('vehicle.volkswagen.t2', parked_transform)
        self.other_actors.append(parked_car) # other_actors[1]

        # NEW: Pedestrian
        pedestrian_waypoint, _ = get_waypoint_in_distance(self._reference_waypoint, self._pedestrian_spawn_distance)
        # Pedestrian initial location (on sidewalk)
        ped_spawn_loc = pedestrian_waypoint.transform.location
        ped_spawn_rot = pedestrian_waypoint.transform.rotation
        
        # Adjust location for sidewalk. Assuming right_vector points towards the sidewalk/edge of road.
        right_vector_ped = pedestrian_waypoint.transform.get_right_vector()
        ped_spawn_loc += carla.Location(x=right_vector_ped.x * 4, y=right_vector_ped.y * 4, z=1.0) # 4m to the right on sidewalk, +1.0 for standing

        # Store the actual spawn transform for the ActorTransformSetter later (on road level)
        self._pedestrian_spawn_transform = carla.Transform(
            carla.Location(ped_spawn_loc.x, ped_spawn_loc.y, ped_spawn_loc.z),
            ped_spawn_rot
        )

        # Initially place pedestrian underground similar to FollowLeadingVehicleWithObstacle
        ped_underground_transform = carla.Transform(
            carla.Location(ped_spawn_loc.x, ped_spawn_loc.y, ped_spawn_loc.z - 500),
            ped_spawn_rot
        )
        pedestrian = CarlaDataProvider.request_new_actor('walker.pedestrian.0001', ped_underground_transform)
        self.other_actors.append(pedestrian) # other_actors[2]
    ...
    def _create_behavior(self):
        ...
        # NEW: Set pedestrian transform on road level at the beginning of the scenario
        # (it was initially placed underground)
        activate_pedestrian = ActorTransformSetter(self.other_actors[2], self._pedestrian_spawn_transform)
        ...
        # NEW: Pedestrian crossing behavior
        # Wait for leading vehicle to stop and ego to be relatively close before pedestrian crosses
        wait_for_pedestrian_trigger = py_trees.composites.Parallel(
            "WaitForPedestrianTrigger",
            policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ALL
        )
        wait_for_pedestrian_trigger.add_child(StandStill(self.other_actors[0], name="LeadingVehicleStopped", duration=2)) # Leading vehicle must be stopped for 2s
        wait_for_pedestrian_trigger.add_child(InTriggerDistanceToVehicle(self.other_actors[0], # Leading vehicle
                                                                         self.ego_vehicles[0], # Ego vehicle
                                                                         distance=25, # Ego is within 25m of leading vehicle
                                                                         name="EgoCloseToLeading"))

        # Pedestrian cross road sequence
        pedestrian_cross_road_sequence = py_trees.composites.Sequence("PedestrianCrossRoadSequence")
        
        # Make pedestrian walk across the road (e.g., 6 meters)
        pedestrian_cross_road_sequence.add_child(KeepVelocity(self.other_actors[2], self._pedestrian_cross_speed))
        pedestrian_cross_road_sequence.add_child(DriveDistance(self.other_actors[2], 6)) # Cross 6 meters
        pedestrian_cross_road_sequence.add_child(StopVehicle(self.other_actors[2], 1.0)) # Stop after crossing
        ...
        sequence.add_child(activate_pedestrian) # Activate pedestrian at start
        sequence.add_child(driving_to_next_intersection)
        sequence.add_child(stop)
        sequence.add_child(wait_for_pedestrian_trigger) # Wait for conditions before pedestrian crosses
        sequence.add_child(pedestrian_cross_road_sequence) # Pedestrian crosses
        sequence.add_child(endcondition)
        sequence.add_child(ActorDestroy(self.other_actors[0]))
        sequence.add_child(ActorDestroy(self.other_actors[1])) # Destroy parked car
        sequence.add_child(ActorDestroy(self.other_actors[2])) # Destroy pedestrian
