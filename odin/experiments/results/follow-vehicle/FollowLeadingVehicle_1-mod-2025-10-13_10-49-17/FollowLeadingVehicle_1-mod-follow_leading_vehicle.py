...
class FollowLeadingVehicle(BasicScenario):
    ...
    def _create_behavior(self):
        ...
        # Part 1: Leading vehicle drives for an initial segment
        initial_drive_segment = py_trees.composites.Parallel(
            "InitialDriveSegment",
            policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ONE)
        initial_drive_segment.add_child(WaypointFollower(self.other_actors[0], self._first_vehicle_speed))
        initial_drive_segment.add_child(DriveDistance(self.other_actors[0], 25))  # Drive for 25 meters

        # Part 2: Leading vehicle performs a sudden brake and brief stop
        intermediate_stop = StopVehicle(self.other_actors[0], self._other_actor_max_brake)
        pause_after_intermediate_stop = TimeOut(1.0)  # Stop for 1 second

        # Part 3: Leading vehicle resumes driving towards the intersection
        resume_drive_to_intersection = py_trees.composites.Parallel(
            "ResumeDriveToIntersection",
            policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ONE)
        resume_drive_to_intersection.add_child(WaypointFollower(self.other_actors[0], self._first_vehicle_speed))
        resume_drive_to_intersection.add_child(InTriggerDistanceToNextIntersection(
            self.other_actors[0], self._other_actor_stop_in_front_intersection))

        # Part 4: Leading vehicle performs the final stop at the intersection
        final_stop = StopVehicle(self.other_actors[0], self._other_actor_max_brake)
        ...
        # Build behavior tree with enhanced complexity
        sequence = py_trees.composites.Sequence("Sequence Behavior")
        sequence.add_child(initial_drive_segment)             # Drive a segment
        sequence.add_child(intermediate_stop)                 # Sudden brake
        sequence.add_child(pause_after_intermediate_stop)    # Brief stop
        sequence.add_child(resume_drive_to_intersection)      # Resume driving to intersection
        sequence.add_child(final_stop)                        # Final stop at intersection
