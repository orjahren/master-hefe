...
class FollowLeadingVehicle(BasicScenario):
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True,
        ...
        # Additional actor parameters for complexity
        self._static_obstacle_location_offset = self._first_vehicle_location + \
            5  # Place static obstacle 35m ahead of trigger point
        # self._static_obstacle_transform = None
    ...
    def _initialize_actors(self, config):
        ...
        """
        # Leading vehicle
        waypoint_lv, _ = get_waypoint_in_distance(
            self._reference_waypoint, self._first_vehicle_location)
        transform_lv = waypoint_lv.transform
        transform_lv.location.z += 0.5
        first_vehicle = CarlaDataProvider.request_new_actor(
            'vehicle.nissan.patrol', transform_lv)
        self.other_actors.append(first_vehicle)

        # Static obstacle (e.g., broken-down vehicle partially blocking the lane)
        static_obstacle_waypoint, _ = get_waypoint_in_distance(
            self._reference_waypoint, self._static_obstacle_location_offset)
        self._static_obstacle_transform = static_obstacle_waypoint.transform
        # self._static_obstacle_transform.location.z += 0.5  # Lift slightly off the ground

        # Rotate the obstacle to make it partially block the lane diagonally
        # Similar to how second_actor in FollowLeadingVehicleWithObstacle gets yaw+90,
        # we'll use yaw+45 for a diagonal block.
        yaw_obstacle = static_obstacle_waypoint.transform.rotation.yaw + 45
        self._static_obstacle_transform.rotation = carla.Rotation(
            static_obstacle_waypoint.transform.rotation.pitch,
            yaw_obstacle,
            static_obstacle_waypoint.transform.rotation.roll
        )

        static_obstacle = CarlaDataProvider.request_new_actor(
            'vehicle.carlamotors.carlacola', self._static_obstacle_transform)
        # static_obstacle.set_simulate_physics(
        # enabled=False)  # Make it a static obstacle
        # This will be self.other_actors[1]
        self.other_actors.append(static_obstacle)
    ...
    def _create_behavior(self):
        ...
        # Set initial transform for the static obstacle
        # self.other_actors[0] is the leading vehicle
        # self.other_actors[1] is the static obstacle
        # static_obstacle_setup = ActorTransformSetter(self.other_actors[1], self._static_obstacle_transform)
        ...
        # sequence.add_child(static_obstacle_setup) # Setup the static obstacle at the beginning
        sequence.add_child(driving_to_next_intersection)
        sequence.add_child(stop)
        sequence.add_child(endcondition)
        # Destroy leading vehicle
        sequence.add_child(ActorDestroy(self.other_actors[0]))
        # Destroy static obstacle
    ...
    def _create_test_criteria(self):
        ...
        static_obstacle_collision_criterion = CollisionTest(
            self.ego_vehicles[0], other_actor=self.other_actors[1], name="CollisionWithStaticObstacle"
        )

        criteria.append(collision_criterion)
        criteria.append(static_obstacle_collision_criterion)
...
class FollowLeadingVehicleWithObstacle(BasicScenario):
    ...
    def _create_behavior(self):
        ...
        obstacle_clear_road.add_child(KeepVelocity(
            self.other_actors[1], self._second_actor_speed))
        ...
        stop_near_intersection.add_child(
            WaypointFollower(self.other_actors[0], 10))
        stop_near_intersection.add_child(
            InTriggerDistanceToNextIntersection(self.other_actors[0], 20))

        driving_to_next_intersection.add_child(WaypointFollower(
            self.other_actors[0], self._first_actor_speed))
